...
static NSString *AG_STICKER_DOWNLOADING_INDICATOR = @"circle.png";
static NSString *AG_STICKER_ICON_DOWNLOADING = @"xiazai.png";
static NSString *AG_STICKER_ICON_SELECTED = @"yellowBorderBackground";
static NSString *AG_STICKER_ICON_NONE = @"cancel.png";
static NSString *AG_STICKER_ICON_DOWNLOADALL = @"download.png";
...
@property(nonatomic, strong) AGIndicatorView *indicatorView;
...
- (UIImageView *)downloadView {
    if (!_downloadView) {
        ...
        _downloadView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:AG_STICKER_ICON_DOWNLOADING]];
...
- (AGIndicatorView *)indicatorView {
    if (!_indicatorView) {
        _indicatorView =
                [[AGIndicatorView alloc] initWithTintColor:[UIColor whiteColor] size:self.imgView.frame.size.width - 8];
...
- (void)setSticker:(AGSticker *)sticker index:(NSInteger)index {
    ...
    if (index == 0) {
        self.imgView.image = [UIImage imageNamed:AG_STICKER_ICON_NONE];
        ...
        self.imgView.image = [UIImage imageNamed:AG_STICKER_ICON_DOWNLOADALL];
        self.downloadView.image = nil;
        ...
        self.downloadView.image = [UIImage imageNamed:AG_STICKER_ICON_DOWNLOADING];
        ...
        if (sticker.isDownload == YES) {
            ...
            if (sticker.downloadState == AGStickerDownloadStateDownoading) {
                self.downloadView.hidden = YES;
...
- (void)startDownload {
    self.sticker.downloadState = AGStickerDownloadStateDownoading;
    self.downloadView.hidden = YES;
    [self.indicatorView startAnimating];
...
- (void)hideBackView:(BOOL)hidden {
    if (hidden) {
        ...
        self.backView.image = [UIImage imageNamed:AG_STICKER_ICON_SELECTED];
    }
}
