...
typedef enum
{
    AGStickerItemTypeFullScreen,  // full-screen display
    AGStickerItemTypeFace,        // Face
    AGStickerItemTypeEdge         // edge
} AGStickerItemType;

typedef enum
{
    AGStickerAlignPositionTop,
    AGStickerAlignPositionLeft,
    AGStickerAlignPositionBottom,
    AGStickerAlignPositionRight,
} AGStickerAlignPosition;

typedef NS_ENUM(NSInteger, AGStickerDownloadState)
{
    AGStickerDownloadStateDownoadNot,//Not downloaded
    AGStickerDownloadStateDownoadDone,//Downloaded
    AGStickerDownloadStateDownoading,//downloading
};

typedef NS_ENUM(NSInteger, AGStickerSourceType)
{
    AGStickerSourceTypeFromKW,//0  AG's downloadURL
    AGStickerSourceTypeFromLocal,//1 your own downloadURL
};

/**
A set of stickers, all the information of a component (such as a nose).
 */
@interface AGStickerItem : NSObject
 ...
 */
@property(nonatomic) AGStickerItemType type;
 ...
 */
@property(nonatomic) AGPosition position;

/**
The edge item parameter
 Edge position（top、bottom、left、right）
 */
@property(nonatomic) AGStickerAlignPosition alignPos;
 ...
 */
@property(nonatomic, strong) NSArray<AGStickerItem *> *items;
 ...
 */
@property(nonatomic, assign) AGStickerDownloadState downloadState;

/**
 The download source type of the sticker
 */
@property(nonatomic, assign) AGStickerSourceType sourceType;
 ...
 */
+ (void)updateStickerAfterDownload:(AGSticker *)sticker DirectoryURL:(NSURL *)dirurl sucess:(void (^)(AGSticker *))sucessed fail:(void (^)(AGSticker *))failed;
