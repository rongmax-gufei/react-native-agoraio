...
- (instancetype)initWithName:(NSString *)name thumbName:(NSString *)thumb download:(BOOL)download DirectoryURL:(NSURL *)dirurl {
    if (self = [super init]) {
        ...
        _downloadState = AGStickerDownloadStateDownoadNot;
        _stickerDir = nil;
        _stickerSound = nil;
        _items = nil;
        ...
        if (download == YES) {
            ...
            _stickerDir = dir;
            _downloadState = AGStickerDownloadStateDownoadDone;
            ...
            AGStickerItem *itemCopy;
            for (NSDictionary *itemDict in itemsDict) {
                AGStickerItem *item = [[AGStickerItem alloc] initWithJSONDictionary:itemDict];
...
- (void)setSourceType:(AGStickerSourceType)sourceType {
    _sourceType = sourceType;
    if (sourceType == AGStickerSourceTypeFromKW) {
        // if you want to use AG's downloadURL, you donot need to do anything
        _downloadURL = nil;
    } else if (sourceType == AGStickerSourceTypeFromLocal) {
...
- (void)setPlayCount:(NSUInteger)count {
    ...
    for (AGStickerItem *item in _items) {
...
+ (void)resetSticker:(AGSticker *)sticker {
