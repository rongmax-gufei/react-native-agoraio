...
@implementation AGStickerManager
{
...
+ (instancetype)sharedManager {
    ...
    dispatch_once(&onceToken, ^{
        _stickersManager = [AGStickerManager new];
    });
...
- (BOOL)getStickersFromJson {
    ...
    //拷贝本地贴纸到沙盒
    NSString *localPath =
            [[[NSBundle mainBundle] pathForResource:@"AGResource" ofType:@"bundle"] stringByAppendingPathComponent:@"stickers"];
...
//    
    ...
    for (NSDictionary *itemDict in newArr) {
        ...
        AGSticker *sticker =
                [[AGSticker alloc] initWithName:[itemDict valueForKey:@"name"] thumbName:[itemDict valueForKey:@"thumb"] download:[[itemDict valueForKey:@"downloaded"] boolValue] DirectoryURL:url];
        if ([[itemDict valueForKey:@"sourceType"] intValue] == 0) {
            sticker.sourceType = AGStickerSourceTypeFromKW;
        } else {
            sticker.sourceType = AGStickerSourceTypeFromLocal;
        }
